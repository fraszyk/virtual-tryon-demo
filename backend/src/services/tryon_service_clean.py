"""Virtual try-on service — Flux.1-Kontext-pro on Azure AI Foundry.

Kontext-pro is BFL's image *editing* model. It takes a photo as `input_image`
and edits it according to the text instruction while preserving the subject's
identity (face, hair, body, pose). This is the correct model for virtual try-on.
...
API call (BFL async):
  POST {endpoint}
  {
    ...
    "input_image": "<base64 PNG of person>",   <- identity source, gets edited
    "image_prompt": "<base64 PNG of garment>", <- style/appearance reference
    "width": 1024, "height": 1536,
    ...
  }
  -> {"id": "...", "polling_url": "..."} or synchronous {"data": [{"b64_json": "..."}]}
"""
...
class TryOnService:
    ...
    def _build_prompt(self, garment_count: int, notes: str | None) -> str:
        garment_ref = (
            "the clothing item in the reference image"
            if garment_count == 1
            else f"all {garment_count} clothing items from the reference images"
        )
        prompt = (
            f"Change only the clothing: dress this exact person in {garment_ref}. "
            "Keep the person's face, skin tone, hair, body shape, and pose completely "
            "identical to the input photo — only the clothes should change. "
            "Make the garment look photorealistic: correct fabric draping, natural fit, "
            "proper proportions. Preserve the original background and lighting."
        )
        if notes:
    ...
    def generate_tryon(
        ...
        # Use first garment as image_prompt reference; if multiple garments exist,
        # we chain: apply each garment sequentially to the output of the previous call.
        ...
        for i in range(n):
            # Start with the person; chain through each garment.
            current_image_b64 = person_b64
            for garment_b64 in garments_b64:
                payload = {
                    "model": Config.AZURE_FLUX_MODEL,
                    # input_image = image to be edited (person → output of previous step)
                    "input_image": current_image_b64,
                    # image_prompt = visual reference for the garment style/appearance
                    "image_prompt": garment_b64,
                ...
                if polling_url:
                    ...
                    current_image_b64 = self._extract_image_b64(result)
                else:
                    current_image_b64 = self._extract_image_b64(submit)

            b64 = current_image_b64
